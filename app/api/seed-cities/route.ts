...
export async function GET(request: Request) {
  ...
  for (const { country, city, lat, lng, count } of CITIES) {
    ...
    for (let i = 0; i < count * 3; i++) {
      ...
      placed.add(key);
      pixels.push({ gridLat: gt, gridLng: gl, countryCode: country, cityName: city, color: ''#1e293b'', isSpecial: false, priceRub: 100, status: ''AVAILABLE'', tier: ''BASIC'' });
      if (pixels.length >= count) break;
    }
    await prisma.pixel.createMany({ data: pixels, skipDuplicates: true });
    total += pixels.length;
    results.push(city + '' ('' + country + ''): '' + pixels.length + '' px'');
  }
  return NextResponse.json({ success: true, total, cities: results.length, details: results });
}
/**
 * Seed script: populates Pixel table with city grid data
 * ~1000 pixels across 63 major cities worldwide
 * Run locally: node scripts/seed-cities.js
 */
...
async function main() {
  console.log(''Seeding city grid pixels...'');
  await prisma.pixel.deleteMany();
  console.log(''Cleared existing pixels'');
  ...
  for (const { country, city, lat, lng, count } of CITIES) {
    ...
    for (let i = 0; i < count * 3; i++) {
      ...
      const jlat = (Math.random() - 0.5) * 2 * spread;
      const jlng = (Math.random() - 0.5) * 2 * spread;
      const gt = Math.round((lat + jlat) * 10);
      const gl = Math.round((lng + jlng) * 10);
      const key = gt + ''_'' + gl;
      if (placed.has(key)) continue;
      placed.add(key);
      pixels.push({
        gridLat: gt, gridLng: gl,
        countryCode: country, cityName: city,
        color: ''#1e293b'', isSpecial: false,
        priceRub: 100, status: ''AVAILABLE'', tier: ''BASIC'',
      });
      if (pixels.length >= count) break;
    }
    await prisma.pixel.createMany({ data: pixels, skipDuplicates: true });
    total += pixels.length;
    console.log(''  '' + city + '' ('' + country + ''): '' + pixels.length + '' px'');
  }
  console.log(''Total: '' + total + '' pixels across '' + CITIES.length + '' cities'');
}
